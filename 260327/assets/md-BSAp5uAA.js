...
`),t.numChars=0;n.length>0&&t.numChars<o.length;){const A=o[t.numChars];t[A]=n.splice(0,r.height);for(let u=0;u<r.height;u++)typeof t[A][u]>"u"?t[A][u]="":t[A][u]=N(t[A][u],u,r.height);t.numChars++}for(;n.length>0;){const A=n.shift();if(!A||A.trim()==="")break;let u=A.split(" ")[0],E;if(/^-?0[xX][0-9a-fA-F]+$/.test(u))E=parseInt(u,16);else if(/^-?0[0-7]+$/.test(u))E=parseInt(u,8);else if(/^-?[0-9]+$/.test(u))E=parseInt(u,10);else throw new Error(`Error parsing data. Invalid data: ${u}`);if(E===-1||E<-2147483648||E>2147483647){const I=E===-1?"The char code -1 is not permitted.":`The char code cannot be ${E<-2147483648?"less than -2147483648":"greater than 2147483647"}.`;throw new Error(`Error parsing data. ${I}`)}t[E]=n.splice(0,r.height);for(let I=0;I<r.height;I++)typeof t[E][I]>"u"?t[E][I]="":t[E][I]=N(t[E][I],I,r.height);t.numChars++}return L[a]=t,r},d.loadedFonts=()=>Object.keys(L),d.clearLoadedFonts=()=>{Object.keys(L).forEach(a=>{delete L[a]})},d.loadFont=async function(a,s){const e=$(a);if(L[e]){const t=L[e].options;return s&&s(null,t),Promise.resolve(t)}try{if(!R.fetchFontIfMissing)throw new Error(`Font is not loaded: ${e}`);const t=await fetch(`${R.fontPath}/${e}.flf`);if(!t.ok)throw new Error(`Network response was not ok: ${t.status}`);const n=await t.text(),l=d.parseFont(e,n);return s&&s(null,l),l}catch(t){const n=t instanceof Error?t:new Error(String(t));if(s)return s(n),null;throw n}},d.loadFontSync=function(a){const s=$(a);if(L[s])return L[s].options;throw new Error("Synchronous font loading is not implemented for the browser, it will only work for fonts already loaded.")},d.preloadFonts=async function(a,s){try{for(const e of a){const t=$(e),n=await fetch(`${R.fontPath}/${t}.flf`);if(!n.ok)throw new Error(`Failed to preload fonts. Error fetching font: ${t}, status code: ${n.statusText}`);const l=await n.text();d.parseFont(t,l)}s&&s()}catch(e){const t=e instanceof Error?e:new Error(String(e));if(s){s(t);return}throw e}},d.fonts=function(a){return new Promise(function(s,e){s(j),a&&a(null,j)})},d.fontsSync=function(){return j},d.figFonts=L,d})(),z_=`flf2a$ 6 5 16 15 13 0 24463 229
