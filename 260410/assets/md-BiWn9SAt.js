...
`),t.numChars=0;n.length>0&&t.numChars<o.length;){const u=o[t.numChars];t[u]=n.splice(0,r.height);for(let L=0;L<r.height;L++)typeof t[u][L]>"u"?t[u][L]="":t[u][L]=S(t[u][L],L,r.height);t.numChars++}for(;n.length>0;){const u=n.shift();if(!u||u.trim()==="")break;let L=u.split(" ")[0],c;if(/^-?0[xX][0-9a-fA-F]+$/.test(L))c=parseInt(L,16);else if(/^-?0[0-7]+$/.test(L))c=parseInt(L,8);else if(/^-?[0-9]+$/.test(L))c=parseInt(L,10);else throw new Error(`Error parsing data. Invalid data: ${L}`);if(c===-1||c<-2147483648||c>2147483647){const E=c===-1?"The char code -1 is not permitted.":`The char code cannot be ${c<-2147483648?"less than -2147483648":"greater than 2147483647"}.`;throw new Error(`Error parsing data. ${E}`)}t[c]=n.splice(0,r.height);for(let E=0;E<r.height;E++)typeof t[c][E]>"u"?t[c][E]="":t[c][E]=S(t[c][E],E,r.height);t.numChars++}return A[s]=t,r},d.loadedFonts=()=>Object.keys(A),d.clearLoadedFonts=()=>{Object.keys(A).forEach(s=>{delete A[s]})},d.loadFont=async function(s,a){const e=$(s);if(A[e]){const t=A[e].options;return a&&a(null,t),Promise.resolve(t)}try{if(!h.fetchFontIfMissing)throw new Error(`Font is not loaded: ${e}`);const t=await fetch(`${h.fontPath}/${e}.flf`);if(!t.ok)throw new Error(`Network response was not ok: ${t.status}`);const n=await t.text(),l=d.parseFont(e,n);return a&&a(null,l),l}catch(t){const n=t instanceof Error?t:new Error(String(t));if(a)return a(n),null;throw n}},d.loadFontSync=function(s){const a=$(s);if(A[a])return A[a].options;throw new Error("Synchronous font loading is not implemented for the browser, it will only work for fonts already loaded.")},d.preloadFonts=async function(s,a){try{for(const e of s){const t=$(e),n=await fetch(`${h.fontPath}/${t}.flf`);if(!n.ok)throw new Error(`Failed to preload fonts. Error fetching font: ${t}, status code: ${n.statusText}`);const l=await n.text();d.parseFont(t,l)}a&&a()}catch(e){const t=e instanceof Error?e:new Error(String(e));if(a){a(t);return}throw e}},d.fonts=function(s){return new Promise(function(a,e){a(z),s&&s(null,z)})},d.fontsSync=function(){return z},d.figFonts=A,d})(),j_=`flf2a$ 6 5 16 15 13 0 24463 229
...
0xCA0  KANNADA LETTER TTHA
 ...
 $\\___/$ @
         @@
         `,z_="/slidev-math-videos/";function K_(){return z_}const X_={class:"my-auto w-full"},J_=u_({__name:"cover",props:{background:{default:""}},setup(m){G();function f(){const N=r_.textSync("hans-green-theme",{font:"Standard"});console.log(N),console.log("🌟🌟 本PPT原作者：%cHans码数理哲","color: #16a34a; font-weight: bold; font-size: 30px;","🌟🌟")}const R=m,I=W_(()=>$_(R.background,!0)),h=`${K_()}live2d/`;function S(N,p){return new Promise((g,b)=>{let C;p==="css"?(C=document.createElement("link"),C.rel="stylesheet",C.href=N):p==="js"&&(C=document.createElement("script"),C.type="module",C.src=N),C&&(C.onload=()=>g(N),C.onerror=()=>b(N),document.head.appendChild(C))})}async function M(){const N=window.Image;window.Image=function(...p){const g=new N(...p);return g.crossOrigin="anonymous",g},window.Image.prototype=N.prototype,await Promise.all([S(`${h}waifu.css`,"css"),S(`${h}waifu-tips.js`,"js")]),initWidget({waifuPath:`${h}waifu-tips.json`,cubism2Path:`${h}live2d.min.js`,cubism5Path:`${h}/cubism5/live2dcubismcore.js`,tools:["hitokoto","asteroids","switch-model","switch-texture","photo","info","quit"],logLevel:"warn",drag:!0})}return K(()=>{r_.parseFont("Standard",j_),f(),M(),console.log(`
%cLive2D%cWidget%c
