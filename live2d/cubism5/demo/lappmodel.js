...
export class LAppModel extends CubismUserModel {
    ...
    startMotion(group, no, priority, onFinishedMotionHandler, onBeganMotionHandler) {
        if (priority == LAppDefine.PriorityForce) {
            ...
            if (this._debugMode) {
                LAppPal.printMessage('[slidev-math-videos] can\'t start motion.');
            }
        ...
        if (this._debugMode) {
            LAppPal.printMessage(`[slidev-math-videos] start motion: [${group}_${no}]`);
        }
    ...
    setExpression(expressionId) {
        ...
        if (this._debugMode) {
            LAppPal.printMessage(`[slidev-math-videos] expression: [${expressionId}]`);
        }
        if (motion != null) {
            ...
            if (this._debugMode) {
                LAppPal.printMessage(`[slidev-math-videos] expression[${expressionId}] is null`);
            }
        }
    }
    setRandomExpression() {
    ...
    preLoadMotionGroup(group) {
        for (let i = 0; i < this._modelSetting.getMotionCount(group); i++) {
            ...
            if (this._debugMode) {
                LAppPal.printMessage(`[slidev-math-videos] load motion: ${motionFileName} => [${name}]`);
            }
            fetch(`${this._modelHomeDir}${motionFileName}`)
                .then(response => {
                    if (response.ok) {
