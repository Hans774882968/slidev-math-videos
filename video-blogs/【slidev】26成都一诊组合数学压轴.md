...
$g(0)=0$ 意味着，在0的**去心邻域**上都要有 $g(x)>0$ ，或者用高中生熟悉的词“**端点效应**”：如果 $g'(0)>0$ 则g的**去心左邻域**必须有小于0的区间，如果 $g'(0)<0$ 则g的**去心右邻域**必须有小于0的区间。所以只能是 $g'(0)=2-a=0 \implies a=2$

做完了？没有！这只是必要条件，还要证明它的充分性。 $g'(0)=0$ ，再求一阶导 $g''(x)=\frac{2}{(x+1)^2}+\frac{1}{2}>0$ ，所以 $g'(x)$ 在 $(-1,0]$ 小于等于0，在 $[0,+\infty)$ 大于等于0，所以 $g(x)$ 在 $(-1,0]$ 单减，在 $[0,+\infty)$ 单增。所以 $a=2$ 合题意
...
这里直接揭晓这条组合恒等式的**命题背景**了：**优惠券收集问题**（Coupon Collector's Problem）。某抽卡游戏卡池里一共有n张卡，抽到每张卡的概率是相等的，每次抽到的卡会放回卡池，用X表示集满所有卡花费的抽卡次数，求X的数学期望。
...
设事件 $A_{i}$ 表示抽k次后没抽到第i张卡，则 $P(X>k)=P(A_{1} \cup A_{2} \cup \dots \cup A_{n})$ 可以用**容斥原理**算。假设必定没抽到t张卡，剩下 $n-t$ 张卡**可能抽到**，那么并集情况数 $C_{n}^t$ ，每种并集情况的概率是 $(1-\frac{t}{n})^k$ ，于是
...
交换求和次序（注：外层无限和，内层有限和，可安全交换。莫比乌斯反演、二项式反演等都会出现的技巧）：
...
## $\sum_{k=1}^{n} C_{n}^k \frac{(-1)^{k+1}}{k}=\sum_{k=1}^{n} \frac{1}{k}$ 参考答案的做法

数学归纳法和参考答案的做法本质上是同一种做法。记 $a_{n}=\sum_{k=1}^{n} C_{n}^k \frac{(-1)^{k+1}}{k}$ ，我们借鉴**数学归纳法**的思想，尝试找到 $a_{n+1}$ 和 $a_{n}$ 的关系。 $a_{n}$ 的组合数是以n为底的，所以由 $C_{n+1}^k=C_{n}^k+C_{n}^{k-1}$ ：

$$
\textcolor{orange}{
\boldsymbol{
a_{n+1}=\sum_{k=1}^{n+1} (C_{n}^k+C_{n}^{k-1})\frac{(-1)^{k+1}}{k}=\sum_{k=1}^{n+1} C_{n}^k\frac{(-1)^{k+1}}{k}+\sum_{k=1}^{n+1} C_{n}^{k-1}\frac{(-1)^{k+1}}{k}
}
}
$$

$C_{n}^{n+1}=0$ ，所以左边的求和式其实就是 $a_{n}$ 。右边的求和式怎么化简呢？

1. 直接考虑二项式定理+求导之类的技巧构造
2. 考虑用 $kC_{n+1}^k=(n+1)C_{n}^{k-1}$ 把组合数从以n为底换成以n+1为底

我们选择“2”：

$$
\sum_{k=1}^{n+1} C_{n}^{k-1}\frac{(-1)^{k+1}}{k}=\sum_{k=1}^{n+1} C_{n+1}^{k}\frac{(-1)^{k+1}}{k}\frac{k}{n+1}=\frac{1}{n+1}\sum_{k=1}^{n+1} C_{n+1}^{k}(-1)^{k+1}
$$

---

## $\sum_{k=1}^{n} C_{n}^k \frac{(-1)^{k+1}}{k}=\sum_{k=1}^{n} \frac{1}{k}$ 参考答案的做法（续）

由**二项式定理**，我们猜测，右边的式子和 $(x-1)^{n+1}\ (x=1)$ 应该长得比较像。

$$
0=(1-1)^{n+1}=\sum_{k=0}^{n+1} (-1)^k C_{n+1}^k \implies -\sum_{k=1}^{n+1} C_{n+1}^{k}(-1)^{k+1}+(-1)^0 C_{n+1}^0=0
$$

$$
\implies \sum_{k=1}^{n+1} C_{n+1}^{k}(-1)^{k+1}=(-1)^0 C_{n+1}^0=1
$$

所以 $a_{n+1}=a_{n}+\frac{1}{n+1}$ ，证毕

扩展思考：参考答案的做法和“期望的尾概率公式和容斥原理”做法有什么深层次联系？

---

## 附录1：推导几何分布

定义：每次抽卡的成功率是p，求第一次抽到卡平均需要多少次。

用期望的定义来算：恰好在第k次抽到卡的概率是 $(1-p)^{k-1}p$ ，所以
...
本期视频还用到一个常见的**容斥原理使用技巧**：把k个集合相交的情况合并在一起算。

::right::

<div class="flex justify-center h-full pt-4">
  <img src="./assets/251226/1-3个集合的容斥原理.png" class="w-80 h-80" alt="1-3个集合的容斥原理.png">
</div>
