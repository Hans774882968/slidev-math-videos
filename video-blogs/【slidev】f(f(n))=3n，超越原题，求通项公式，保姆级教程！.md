...
<MovingWatermark />
<AutoSlide :timeList="[13, 33, 71, [59, 4], 12.5, 81]" />

## f(f(n))=3n，求这道优雅数列题的通项公式，保姆级教程！
...
B：假设 $a_{n} \leq n$ 有： $a_{a_{n}} \leq a_{n} \leq n$ ，但 $a_{a_{n}} = 3n > n$ ，矛盾。B正确。

C：手推一下。由B， $a_{a_{1}} = 3 > a_{1}$ 。令 $a_{1} = 1$ ，显然不对，所以 $a_{1} = 2$ ，进而推出 $a_{a_{1}}=a_{2}=3$ 。然后 $a_{a_{2}} = 3*2 = 6 = a_{3}$ ，C错误。

D：我们注意到 $2025 = 81*25 = 3^4*25$ ，所以希望得到 $a_{3n}$ 和 $a_{n}$ 的关系。突破口在：多嵌套1层。设 $a_{n} = t$ 则 $a_{a_{n}} = a_{t} = 3n$ ， $a_{a_{a_{n}}} = a_{3n} = a_{a_{t}} = 3t$ ，这就得到了 $a_{3n} = 3a_{n}$ 。所以D正确。**总结**：3层嵌套的**两个视角**：2+1（ $a_{3n}$ ）=1+2 （ $a_{a_{t}}$ ）
...
3. $2*3^k\leq n\leq 3^{k+1}$ ：受手推过程启发，思路仍然是**多嵌套一层**。注意到“2”的通项公式是加 $3^k$ ，这启发我们设 $t=n-3^k$ ，于是由“2”得 $a_{t}=n$ ，再套一层得 $a_{n}=a_{a_{t}}=3t=3(n-3^k)=3n-3^{k+1}$

注：严谨的解题过程要写成**数学归纳法**。假设 $k=x$ 时通项公式成立，在 $k=x+1$ 时依次证明1~3成立
