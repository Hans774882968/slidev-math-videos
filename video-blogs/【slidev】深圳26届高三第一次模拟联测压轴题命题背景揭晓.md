...
<MovingWatermark />
<AutoSlide :timeList="[22, 30, 66, 41, 60.5, 30, 35, 20.5, 14, 17]" />

## 上期视频极值点偏移问题的命题背景揭晓！给各位出道导数压轴题~
...
一起站在<span class="text-6xl">命题人</span>的高度出<span class="text-6xl">导数压轴题</span>！
</div>
...
显然 $f(1)=0$ ，所以 $f(x_2)+f(x_1)=(f(x_2)-f(1))-(f(1)-f(x_1))>0$ 。这表示，我们要证明的就是 $f'(x)$ 从 $x_{1}$ 到1的积分小于从1到 $x_{2}$ 的积分。
...
---
layout: image
image: ./assets/251128/251128-3-命题背景图示.png
...
思路主要有：找到类似于三次函数的`G(x)`再求导得单峰的`f(x)`；先找`f(x)`再积分求出`G(x)`
...
2. 一开始我没有加最后那句提示，得到的是多项式。后来加了提示，发现能拿到带`ln`的式子
...
不给任何提示，直接问LLM，发现主流大模型都没有能力解决这个问题！

> 大佬，你是一名数学科研工作者，精通分析方向。<题干>
...
注：这两页PPT说的函数f指的是上一页题干的f

1. $f(x_{1})+f(x_{2})$ 随a变化的曲线：随a单减，且始终大于等于0
2. $f(x_{1})+f(6-x_{1})$ 随a变化的曲线：同样随a单减，始终大于等于0，且始终小于等于 $f(x_{1})+f(x_{2})$
3. $x_{1}+x_{2}$ 随a变化的曲线：随a单减，大致呈线性，且始终大于等于6（2倍极值点）

::right::

<div class="flex justify-center h-full pt-4">
  <img src="./assets/251128/251128-1-随a变化的曲线合集.png" class="w-100" alt="251128-1-随a变化的曲线合集.png">
</div>
...
$f(x)+f(6-x)$ 刚好把a消掉，是一个固定的函数。它关于极值点对称，且在极值点右侧是单增的，极值点处为最小值0。它的导数 $f'(x)-f'(6-x)$ 就是极值点偏移模型所构造的函数。

<div class="flex justify-center">
  <img src="./assets/251128/251128-2-f(x)+f(6-x).png" class="h-86" alt="251128-2-f(x)+f(6-x).png">
</div>
...
思考：修改下面的f和G，使得极值点为 $e^n$ ，其中n为正整数
