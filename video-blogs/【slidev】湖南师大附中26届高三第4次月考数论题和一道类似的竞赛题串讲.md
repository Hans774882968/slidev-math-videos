...
2. **欧拉函数**：
3. **欧拉定理**：
4. 本期视频不直接使用欧拉定理，而是使用欧拉定理揭示的事实： $a^n\ mod\ m$ 是具有周期性的，而且很多时候这个周期很小
...
| ----------- | ----- | --- | --- | ----- | --- | --- | ----- | --- | --- | ----- | --- | --- |
| $3^n$       | **3** | 9   | 1   | **3** | 9   | 1   | **3** | 9   | 1   | **3** | 9   | 1   |
| $4^n$       | **4** | 3   | 12  | 9     | 10  | 1   | **4** | 3   | 12  | 9     | 10  | 1   |
| $4^n+3^n+6$ | **0** | 5   | 6   | 5     | 12  | 8   | **0** | 5   | 6   | 5     | 12  | 8   |

1. 加粗的数据标识周期的开始
2. $3^n,\ 4^n$ 模13的周期分别是 $6,\ 3$ ，所以 $4^n+3^n+6$ 模13的周期就是 $lcm(6,\ 3)=6$
...
所以当n是奇数， $3^n$ 模4为3，当n是偶数， $3^n$ 模4余1。所以 $T_{2026}$ 可以写成
...
8的幂大于等于2时都是16的倍数，都没有影响，所以

$$
T_{2026}\equiv-1013+\frac{3^{2027}-3}{8}\equiv-1013+3(C_{1013}^1+C_{1013}^2 8)\ (mod\ 16)
$$

注意到 $8C_{1013}^2=8*1013*506$ 是16的倍数，所以所求为 $T_{2026} \equiv 2026 \equiv 10\ (mod\ 16)$
...
回顾前置芝士“4”，9的幂模16的周期一定是 $\phi(16)=8$ 的因数。所以我们可以轻松把这个周期数列写出来： $9,\ 1,\ 9,\ 1,\ \dots$ 。我们发现周期为2。所以
