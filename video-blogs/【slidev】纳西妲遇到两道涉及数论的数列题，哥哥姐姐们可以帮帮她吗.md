...
自编两道涉及数论的数列水题，灵感：《高中数学竞赛培优教程专题讲座》第三版第36页例2.18
...
$a_{n+1}$ 和 $a_{n}$ 的系数只差一个 $\frac{3}{2}$ ，所以我们两边同时除以 $(\frac{3}{2})^n$ 就能拿到 $b_{n}$ ：
...
$2^t$ 和 $3^t$ 互素，所以可以用**中国剩余定理**（CRT）解出同余方程组的解，然后验证是否满足第三个条件。
